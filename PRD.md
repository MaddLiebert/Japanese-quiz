...
* **[EXISTING]** Mic popup while listening (`MicOverlay` in both `SpeakSession` and `PoemSession`): pulsing mic icon + real-time audio spectrum (`useMicLevel` — getUserMedia + AnalyserNode, bars from pure `micSpectrum.js`) + live interim transcript from the Web Speech API + a Cancel button. It is a compact bottom sheet (no full-screen scrim) so the question stays readable; the page gains bottom clearance while listening and `PoemSession` auto-scrolls the line being spoken to viewport center. Listen/Skip buttons are disabled while the mic is on (no double-skip, no TTS bleed into the mic).
